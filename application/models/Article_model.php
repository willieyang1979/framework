...
class Article_model extends CI_Model
{
    ...
    public function get_info($where = NULL, $limit = null, $field='*',$order_by=null,$d_or_a = null)
    {
        ...
        if ($order_by && $d_or_a)
        {
            ...
            $this->db->order_by("article_sort","desc");
        }
        if ($limit)
        {
            $this->db->limit($limit,0);
        }
        return $this->db->get('osa_article')->result_array();
    }
    
    
    public function get_home_articles($menu,$limit = null)
    {
        $this->db->select('*');
        $this->db->where('group_id',$this->config->item('web_id'));
        $this->db->where('is_publish','1');
        $this->db->where('show_in_home','1');
        $this->db->where('article_type',$menu);
        $this->db->order_by("article_sort","desc");
        $this->db->limit($limit,0);
